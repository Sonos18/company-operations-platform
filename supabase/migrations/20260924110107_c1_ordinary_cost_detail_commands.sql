...
begin
  if jsonb_typeof(target_input)<>'object' or not (target_input ?& array['expectedVersion','reason','changes']) or exists (select 1 from jsonb_object_keys(target_input) key where key not in ('expectedVersion','reason','changes'))
    or jsonb_typeof(target_input->'expectedVersion')<>'number' or (target_input->>'expectedVersion') !~ '^\\d+$'
  ...
  if (v_changes ? 'description' and (jsonb_typeof(v_changes->'description')<>'string' or btrim(v_changes->>'description')=''))
    ...
    or (v_changes ? 'retentionRateBps' and (jsonb_typeof(v_changes->'retentionRateBps') not in ('number','null') or (jsonb_typeof(v_changes->'retentionRateBps')='number' and ((v_changes->>'retentionRateBps') !~ '^\\d+$' or (v_changes->>'retentionRateBps')::integer not between 0 and 10000))))
