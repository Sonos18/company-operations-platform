...
begin
  ...
  if not exists (
    ...
    where item.id=v_detail.project_cost_item_id and item.tenant_id=target_tenant_id and item.company_id=target_company_id
      and item.publication_state='published' and category.posting_strategy='ordinary_detail'
  ) then raise exception using errcode='P0001',message='COST_DETAIL_PUBLISH_NOT_READY'; end if;
  return v_detail;
end;
$$;

create function private.c1_detail_expected_version(target_input jsonb)
...
begin
  v_context:=private.c1_detail_context(target_company_id,'cost.manage'); v_expected_version:=private.c1_detail_expected_version(target_input); v_detail:=private.c1_detail_require_scope(target_id,(v_context->>'tenantId')::uuid,target_company_id);
...
create function private.c1_publish_project_cost_detail(target_company_id uuid,target_id uuid,target_input jsonb,target_idempotency_key uuid,target_request_id uuid)
returns jsonb language plpgsql security definer set search_path='' as $$
declare v_context jsonb; v_tenant_id uuid; v_actor_id uuid; v_detail public.project_cost_item_details%rowtype; v_hash text; v_receipt public.cost_command_receipts%rowtype; v_expected_version bigint;
begin
  ...
  v_expected_version:=private.c1_detail_publish_expected_version(target_input);v_hash:=encode(extensions.digest(convert_to(jsonb_build_object('id',target_id,'expectedVersion',v_expected_version),'UTF8'),'sha256'),'hex');v_receipt:=private.c1_detail_receipt(v_tenant_id,target_company_id,v_actor_id,'project_cost_detail.publish',target_idempotency_key,v_hash);if v_receipt.id is not null then return private.c1_detail_ack(v_receipt.result_resource_id,true);end if;
  v_detail:=private.c1_detail_require_scope(target_id,v_tenant_id,target_company_id);if v_detail.publication_state='published' then raise exception using errcode='P0001',message='COST_DETAIL_ALREADY_PUBLISHED';end if;if v_detail.version is distinct from v_expected_version then raise exception using errcode='P0001',message='VERSION_CONFLICT';end if;if v_detail.amount_text is null then raise exception using errcode='P0001',message='COST_DETAIL_PUBLISH_NOT_READY';end if;perform private.c1_detail_validate_linked_sources(v_detail.id,v_tenant_id,target_company_id);
...
revoke all on function private.c1_detail_context(uuid,text), private.c1_detail_receipt(uuid,uuid,uuid,text,uuid,text), private.c1_detail_ack(uuid,boolean), private.c1_detail_require_scope(uuid,uuid,uuid), private.c1_detail_expected_version(jsonb), private.c1_detail_publish_expected_version(jsonb), private.c1_detail_replace_sources(public.project_cost_item_details,uuid[]), private.c1_detail_validate_linked_sources(uuid,uuid,uuid), private.c1_detail_validate_correction_input(jsonb), private.c1_detail_validate_correction_shape(public.project_cost_item_details,jsonb), private.c1_create_project_cost_detail_draft(uuid,jsonb,uuid,uuid), private.c1_update_project_cost_detail_draft(uuid,uuid,jsonb,uuid), private.c1_prepare_project_cost_detail_financials(uuid,uuid,jsonb,uuid), private.c1_publish_project_cost_detail(uuid,uuid,jsonb,uuid,uuid), private.c1_create_and_publish_project_cost_detail(uuid,jsonb,uuid,uuid), private.c1_correct_published_project_cost_detail(uuid,uuid,jsonb,uuid,uuid), private.c1_read_project_cost_detail_draft(uuid,uuid,boolean), private.c1_list_project_cost_detail_drafts(uuid,uuid,boolean) from public, anon, authenticated;
revoke all on function public.c1_create_project_cost_detail_draft(uuid,jsonb,uuid,uuid), public.c1_update_project_cost_detail_draft(uuid,uuid,jsonb,uuid), public.c1_prepare_project_cost_detail_financials(uuid,uuid,jsonb,uuid), public.c1_publish_project_cost_detail(uuid,uuid,jsonb,uuid,uuid), public.c1_create_and_publish_project_cost_detail(uuid,jsonb,uuid,uuid), public.c1_correct_published_project_cost_detail(uuid,uuid,jsonb,uuid,uuid), public.c1_read_project_cost_detail_draft(uuid,uuid), public.c1_read_project_cost_detail_draft_operational(uuid,uuid), public.c1_list_project_cost_detail_drafts(uuid,uuid), public.c1_list_project_cost_detail_drafts_operational(uuid,uuid) from public, anon, authenticated;
