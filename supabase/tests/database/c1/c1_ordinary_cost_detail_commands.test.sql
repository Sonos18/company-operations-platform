...
begin
  ...
  insert into public.source_selections(id,tenant_id,company_id,source_version_id,import_run_id,locator,locator_key,mapping_state,reviewed_mapping,mapped_project_id,observed_labels,raw_values,unresolved_issues,created_by) values
    ('c1d10000-0000-4000-8000-000000000407',tenant_id,company_id,'c1d10000-0000-4000-8000-000000000406','c1d10000-0000-4000-8000-000000000401',jsonb_build_object('sheet','fixture'),'fixture!A1','confirmed',jsonb_build_object('projectId',project_id),project_id,array['fixture'],array['1'],array[]::text[],actor_id),
    ('c1d10000-0000-4000-8000-000000000408',tenant_id,company_id,'c1d10000-0000-4000-8000-000000000406','c1d10000-0000-4000-8000-000000000401',jsonb_build_object('sheet','blocked'),'fixture!A2','confirmed',jsonb_build_object('projectId',project_id),project_id,array['blocked'],array['1'],array[]::text[],actor_id);
  insert into public.source_reported_figures(id,tenant_id,company_id,source_selection_id,import_run_id,figure_identity,label,raw_value_text,value_state,amount_text,amount,currency_code,metric_kind,basis,rounding_basis,period_basis,mapping_state,reviewed_mapping,project_id,scope_kind,scope_description,confirmation,status,created_by,shared_by,shared_at) values
    ...
    ('c1d10000-0000-4000-8000-000000000503',tenant_id,company_id,'c1d10000-0000-4000-8000-000000000408','c1d10000-0000-4000-8000-000000000401',repeat('7',64),'blocked figure','1','known','1',1,'VND','cost_total','net','exact','unknown','confirmed',jsonb_build_object('projectId',project_id),project_id,'whole_project','fixture','confirmed_external','shared',actor_id,actor_id,now());
end;
$$;
...
select is((public.c1_prepare_project_cost_detail_financials('c1d10000-0000-4000-8000-000000000020',(select (result->>'id')::uuid from c1d_sibling),jsonb_build_object('expectedVersion',0,'amount','1.0000','sourceFigureIds',jsonb_build_array('c1d10000-0000-4000-8000-000000000503')),'c1d10000-0000-4000-8000-000000000717')->>'version'),'1','prepared sibling persists an explicit shared source link');
reset role;
select throws_ok(
...
insert into public.source_review_issues(id,tenant_id,company_id,source_selection_id,import_run_id,issue_identity,issue_kind,impact,description,status,opened_by) values('c1d10000-0000-4000-8000-000000000504','c1d10000-0000-4000-8000-000000000010','c1d10000-0000-4000-8000-000000000020','c1d10000-0000-4000-8000-000000000408','c1d10000-0000-4000-8000-000000000401',repeat('1',64),'other','blocks_normalization','fixture blocking review','open','c1d10000-0000-4000-8000-000000000901');
set local role authenticated;
...
select is((select amount_text from public.project_cost_items where id=(select (result->>'projectCostItemId')::uuid from c1d_result)),'15.0000','direct source payload variants retain exact aggregate accounting');
