...
describe('C1 Cloud DEV migration rehearsal runner', () => {
  ...
  it('rejects a missing accounting-write migration before Cloud access', () => {
    expect(() => readC1MigrationSql(migrationRoot(migrationNames.slice(0, 10)))).toThrow('C1 migration rehearsal requires exactly one migration for _c1_accounting_write_finalize_validation_fix.sql')
  })
