...
describe('C1 accounting write target', () => {
  ...
  it('keeps verified evidence finalization behind a service-role-only transition',()=>{
    const sql=phaseMigration('_c1_accounting_write_finalize_server_boundary.sql')
    expect(sql).toContain('function public.c1_finalize_cost_evidence_server')
    expect(sql).toContain("auth.role() is distinct from 'service_role'")
    expect(sql).toContain("set_config('request.jwt.claims'")
    expect(sql).toContain('private.c1_finalize_cost_evidence(target_company_id,target_id,target_input,target_idempotency_key,target_request_id)')
    expect(sql).toContain('revoke all on function public.c1_finalize_cost_evidence(uuid,uuid,jsonb,uuid,uuid) from public,anon,authenticated,service_role')
    expect(sql).toContain('grant execute on function public.c1_finalize_cost_evidence_server(uuid,uuid,uuid,jsonb,uuid,uuid) to service_role')
    expect(sql).not.toContain('grant execute on function public.c1_finalize_cost_evidence_server(uuid,uuid,uuid,jsonb,uuid,uuid) to authenticated')
  })
})
