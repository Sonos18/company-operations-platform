...
function query(result: { data: unknown, error: unknown }, onEq?: (column: string, value: unknown) => void) {
  const value = {
    select: () => value,
    eq: (column: string, filter: unknown) => { onEq?.(column, filter); return value },
    order: () => value,
    limit: () => value,
...
describe('Stage 01 Decision repository', () => {
  it('reads the runtime-bound definition, related contacts, and ordered decision-cycle history', async () => {
    ...
    const taxonomies = Object.fromEntries(taxonomyKeys.map(key => [key, [{ code: key, label: `Label ${key}`, semanticKey: `internal.${key}`, ...(key === 'lead_source' ? { behavior: { requiresReferrer: true } } : {}) }]]))
    ...
    const from = vi.fn((table: string) => {
      if (table === 'stage01_decision_cycles') return query({ data: [cycle(cycleOne, 1), cycle(cycleTwo, 2)], error: null })
      if (table === 'workflow_definition_snapshots') return query({ data: { definition: { taxonomies, criteria, capabilities: {} } }, error: null }, (column, value) => { if (column === 'id') snapshotLookups.push(value) })
    ...
    expect(detail?.configuration.taxonomies.customer_type[0]).toEqual({ code: 'customer_type', label: 'Label customer_type' })
