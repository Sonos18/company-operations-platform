...
import { canonicalizeManifest, decideReplay, validateReviewedImport } from '../../../server/features/costs/imports/import-manifest'

const company = 'c1000000-0000-4000-8000-000000000020'
const base = { schemaVersion: '1.2', workbookFamily: 'synthetic-ledger-v1', adapter: { id: 'synthetic-ledger', version: '1.0.0' }, targetCompanyId: company, inputs: [{ fileIdentity: 'source', sha256: 'a'.repeat(64), originalFilename: 'source.xlsx' }], sections: [{ id: 's1', inputSha256: 'a'.repeat(64), locator: { kind: 'cell_range', sheetName: ' 日本 ', range: 'B1:A2' }, mappingState: 'pending', observedLabels: ['x'], rawValues: ['0'], unresolvedIssues: [] }], expected: { sources: 1, versions: 1, sections: 1, figures: 0, reviewIssues: 0 } }
...
describe('controlled-import manifest helpers', () => {
  ...
  it('distinguishes retry identity from request correlation and detects payload conflict', () => {
    expect(decideReplay({ requestId: 'one', idempotencyKey: 'k', payloadDigest: 'a', existing: { idempotencyKey: 'k', payloadDigest: 'a' } })).toBe('replay')
    expect(decideReplay({ requestId: 'two', idempotencyKey: 'k', payloadDigest: 'b', existing: { idempotencyKey: 'k', payloadDigest: 'a' } })).toBe('IDEMPOTENCY_CONFLICT')
  })
})
