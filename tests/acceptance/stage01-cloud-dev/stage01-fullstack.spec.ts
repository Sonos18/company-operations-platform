...
test('B4-S01/S04/S06/S08 completes the real acceptance-company journey and preserves canonical history', async ({ page }) => {
  ...
  const completedDecisionCycle = decisionCycleHistory.locator('details').filter({ has: page.getByText('Chu kỳ #1 · Đã ghi nhận quyết định', { exact: true }) })
  await completedDecisionCycle.locator('summary').click()
  await expect(completedDecisionCycle.getByText(decisionRationale, { exact: false })).toBeVisible()
  ...
  const s08CurrentDecisionCycle = s08CycleHistory.locator('details').filter({ has: page.getByText('Chu kỳ #2 · Chưa ghi nhận quyết định', { exact: true }) })
  await expect(s08CurrentDecisionCycle.locator('summary')).toContainText(`Kích hoạt lại: ${reactivationReason}`)
  const s08CompletedDecisionCycle = s08CycleHistory.locator('details').filter({ has: page.getByText('Chu kỳ #1 · Đã ghi nhận quyết định', { exact: true }) })
  await s08CompletedDecisionCycle.locator('summary').click()
  await expect(s08CompletedDecisionCycle.getByText(s08DecisionRationale, { exact: false })).toBeVisible()
