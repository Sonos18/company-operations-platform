...
export const controlledImportManifestSchema = z.object({
  ...
  const inputs = new Map(value.inputs.map(input => [input.fileIdentity, input.sha256])); const sources = new Set(value.sources.map(source => source.id)); const versions = new Map(value.sourceVersions.map(version => [version.id, version])); const sections = new Set(value.sections.map(section => section.id))
  for (const sourceVersion of value.sourceVersions) { if (!sources.has(sourceVersion.sourceId) || inputs.get(sourceVersion.inputFileIdentity) !== sourceVersion.inputFileSha256) ctx.addIssue({ code: 'custom', message: 'invalid source version reference' }) }
