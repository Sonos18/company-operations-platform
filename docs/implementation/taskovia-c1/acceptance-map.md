...
**Status after v1.2 transition:** P1 is complete. The P2 v1.1 runtime-upload candidate is superseded before Cloud application. P2.1 is frozen and P2.3 has a local deterministic persistence candidate plus prepared synthetic SQL fixtures. After the original read-only baseline failed with SQLSTATE `22P02`, an explicit continuation allowed one corrected baseline; it was clean. The authorized dry-run proposed only P2.3, but the single push failed with SQLSTATE `42703` because the reviewed migration indexed missing `source_review_issues.created_at`. Bounded outcome inspection showed no migration-history, schema, or RPC residue. The local candidate now uses declared `opened_at` and has a structural all-index regression, but no P2.3 migration or fixture ran against a database. Database/RLS/concurrency rows remain partial. P3–P6 remain not started. See [the execution review](phase-reports/p2-3-cloud-review.md).
