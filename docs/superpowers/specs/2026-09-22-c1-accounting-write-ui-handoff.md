...
The server downloads through the authenticated client and finalizes only when stored Content-Type, size, SHA-256, and byte-format identity all match the declaration. The final database transition is internal to Nitro and is not callable by an ordinary authenticated Supabase client; the UI must continue to call only this HTTP endpoint. PDF, PNG, JPEG, and legacy XLS use their required file signatures. XLSX must be a valid bounded ZIP containing `[Content_Types].xml`, `xl/workbook.xml`, and the spreadsheet workbook content type; an arbitrary ZIP or archive with unsafe entry-count, declared-size, aggregate-expansion, or metadata compression-ratio values is rejected before content expansion. The server streams the content-types metadata under both declared and actual byte ceilings, requires exact size agreement, and does not load worksheets, shared strings, embedded media, or workbook contents. This verifies the declared file-format identity, not document safety, business meaning, or workbook financial contents. A mismatch returns `EVIDENCE_UPLOAD_MISMATCH` and creates no link. An exact finalize retry reaches its receipt even though `cost.prepare` alone cannot read finalized metadata. The request and response shapes above are unchanged; the UI does not send verification metadata, credentials, or attestations.
