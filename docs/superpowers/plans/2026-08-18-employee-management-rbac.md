...
| Cloud DEV non-mutating preflight | `pnpm db:dev:auth-check`, `db:dev:link`, `db:dev:status`, `db:dev:dry-run`, and both advisor commands | Root reran status/dry-run after `8ecaf19`: succeeded on canonical ref `ykrurrumqlsxnqfqunjc` with exactly five forward migrations, `seeds=[]`, and CLI `roles=[]`; no push, reset, or seed was run. CLI `roles=[]` is not the database role catalog. |
...
| Catalog bootstrap readiness | Forward migration `20260820042507_bootstrap_vqh_employee_rbac_catalog.sql`; focused Cloud DEV/config units | Prepared but not applied: the fifth migration contains catalog/reference data only—7 departments, 8 system roles, 34 permissions, and 71 mappings—and contains no Auth identities, employees, memberships, role assignments, or local fixture seed. Canonical checks require that catalog plus an active normalized company admin. Cloud DEV dry-run/push remain separately authorized. |
...
The remaining deployment gate is separately authorized Cloud DEV migration application. An authorized operator then runs the sentinel-replaced `docs/development/sql/onboard-vqh-dev-admin.sql` through the restricted Cloud DEV SQL Editor or controlled role without committing identity data, followed by the mandatory no-Docker `db:dev:status`, `db:dev:types`, `db:dev:canonical-check`, `db:dev:rls-smoke`, and both advisor commands. `db:dev:test` is a separately recorded optional Docker/container-capable pgTAP check. Completion checklist items remain unchecked until the pending schema is applied and those database gates produce evidence.
