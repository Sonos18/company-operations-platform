...
<style scoped>
.journey-stage-card { width: 100%; height: 54%; min-height: 0; overflow: hidden; border: 1px solid var(--journey-border); border-radius: var(--journey-radius); background: var(--journey-surface); color: var(--journey-foreground); transition: opacity var(--journey-motion), transform var(--journey-motion), box-shadow var(--journey-motion); }
.journey-stage-card:not(.is-focused) { transform: scale(.94); opacity: .68; }
.journey-stage-card.is-focused { height: 84%; transform: translateY(-4px) scale(1.015); box-shadow: 0 18px 36px color-mix(in srgb, var(--journey-primary) 22%, transparent); }
