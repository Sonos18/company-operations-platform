...
export function decideReplay({ idempotencyKey, payloadDigest, existing }: { requestId: string; idempotencyKey: string; payloadDigest: string; existing?: { idempotencyKey: string; payloadDigest: string } }) { return !existing ? 'new' : existing.idempotencyKey === idempotencyKey && existing.payloadDigest === payloadDigest ? 'replay' : 'IDEMPOTENCY_CONFLICT' }
