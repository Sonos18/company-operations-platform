import { z } from 'zod'
import type { CostSourceFigure, CostSourceFiguresQuery } from '../../../shared/schemas/costs/source-read-model'
import { AppApiError } from '../../utils/api-error'
import type { UserSupabaseClient } from '../../utils/supabase-client'

const probeRunId = '00000000-0000-4000-8000-000000000001'
const figureColumns = `id, tenant_id, company_id, label, raw_value_text, value_state, amount_text, currency_code, basis, scope_kind, scope_description, mapping_state, project_id, engagement_id, party_id, confirmation, created_at, source_selections!inner(id, locator, mapped_project_id, mapped_engagement_id, mapped_party_id, accounting_source_versions!inner(id, version_no, original_filename, accounting_sources!inner(id, code, title, source_system)), projects!source_selections_mapped_project_id_tenant_id_company_id_fkey(id, code, name), project_engagements!source_selections_mapped_engagement_id_tenant_id_company_id_project_id_fkey(id, code, name), business_parties!source_selections_mapped_party_id_tenant_id_company_id_fkey(id, code, display_name), source_review_issues(issue_kind, impact, description, status)), projects!source_reported_figures_project_id_tenant_id_company_id_fkey(id, code, name), project_engagements!source_reported_figures_engagement_id_tenant_id_company_id_project_id_fkey(id, code, name), business_parties!source_reported_figures_party_id_tenant_id_company_id_fkey(id, code, display_name)`
