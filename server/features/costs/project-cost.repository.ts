...
const detailColumns = 'id,tenant_id,company_id,project_cost_item_id,line_no,detail_kind,description,quantity_text,unit_code,unit_price_text,amount_text,relevant_date,reference,note,version,created_by,created_at,updated_at'
const detailRowSchema = z.object({
  id: z.string().uuid(), tenant_id: z.string().uuid(), company_id: z.string().uuid(), project_cost_item_id: z.string().uuid(), line_no: z.number().int().positive(), detail_kind: z.enum(['opening_balance', 'line_item']), description: z.string().trim().min(1), quantity_text: z.string().nullable(), unit_code: z.string().nullable(), unit_price_text: z.string().nullable(), amount_text: z.string(), relevant_date: z.string().date().nullable(), reference: z.string().nullable(), note: z.string().nullable(), version: z.number().int().nonnegative(), created_by: z.string().uuid(), created_at: z.string().datetime({ offset: true }), updated_at: z.string().datetime({ offset: true }),
}).strict()
