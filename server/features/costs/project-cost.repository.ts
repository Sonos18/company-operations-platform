...
export class ProjectCostRepository implements ProjectCostDataRepository {
  ...
  async draftManagementMetadata(context: ProjectCostRequestContext) {
    const { data, error } = await this.client.rpc('c1_read_project_cost_draft_management_metadata', { target_company_id: context.companyId })
    if (error) return rpcError(error)
    const result = projectCostDraftManagementMetadataSchema.safeParse(data)
  ...
  private async detailCommand(rpc: 'c1_create_project_cost_detail_draft' | 'c1_update_project_cost_detail_draft' | 'c1_prepare_project_cost_detail_financials' | 'c1_publish_project_cost_detail' | 'c1_create_and_publish_project_cost_detail' | 'c1_correct_published_project_cost_detail', context: ProjectCostRequestContext, args: Record<string, unknown>) {
    const { data, error } = await this.client.rpc(rpc, { target_company_id: context.companyId, ...args, target_request_id: context.requestId })
    if (error) return rpcError(error)
    const result = projectCostDetailCommandAckSchema.safeParse(data)
    return result.success ? result.data : fail('Không thể cập nhật chi tiết Project Cost.')
  }

  createDetailDraft(context: ProjectCostRequestContext, input: CreateProjectCostDetailDraftInput, idempotencyKey: string) { return this.detailCommand('c1_create_project_cost_detail_draft', context, { target_input: input, target_idempotency_key: idempotencyKey }) }
  updateDetailDraft(context: ProjectCostRequestContext, id: string, input: UpdateProjectCostDetailDraftInput) { return this.detailCommand('c1_update_project_cost_detail_draft', context, { target_id: id, target_input: input }) }
  prepareDetailFinancials(context: ProjectCostRequestContext, id: string, input: PrepareProjectCostDetailFinancialsInput) { return this.detailCommand('c1_prepare_project_cost_detail_financials', context, { target_id: id, target_input: input }) }
  publishDetail(context: ProjectCostRequestContext, id: string, expectedVersion: number, idempotencyKey: string) { return this.detailCommand('c1_publish_project_cost_detail', context, { target_id: id, target_expected_version: expectedVersion, target_idempotency_key: idempotencyKey }) }
  createAndPublishDetail(context: ProjectCostRequestContext, input: CreateAndPublishProjectCostDetailInput, idempotencyKey: string) { return this.detailCommand('c1_create_and_publish_project_cost_detail', context, { target_input: input, target_idempotency_key: idempotencyKey }) }
  correctPublishedDetail(context: ProjectCostRequestContext, id: string, input: CorrectPublishedProjectCostDetailInput, idempotencyKey: string) { return this.detailCommand('c1_correct_published_project_cost_detail', context, { target_id: id, target_input: input, target_idempotency_key: idempotencyKey }) }

  private async detailRead<T>(rpc: 'c1_read_project_cost_detail_draft' | 'c1_list_project_cost_detail_drafts' | 'c1_read_project_cost_detail_draft_operational' | 'c1_list_project_cost_detail_drafts_operational', context: ProjectCostRequestContext, args: Record<string, unknown>, schema: z.ZodType<T>) {
    const { data, error } = await this.client.rpc(rpc, { target_company_id: context.companyId, ...args })
    if (error) return rpcError(error)
    const result = schema.safeParse(data)
    return result.success ? result.data : fail('Không thể đọc chi tiết bản nháp Project Cost.')
  }

  detailDraft(context: ProjectCostRequestContext, id: string) { return this.detailRead('c1_read_project_cost_detail_draft', context, { target_id: id }, projectCostDetailDraftSchema) }
  listDetailDrafts(context: ProjectCostRequestContext, projectId: string) { return this.detailRead('c1_list_project_cost_detail_drafts', context, { target_project_id: projectId }, z.array(projectCostDetailDraftSchema)) }
  operationalDetailDraft(context: ProjectCostRequestContext, id: string) { return this.detailRead('c1_read_project_cost_detail_draft_operational', context, { target_id: id }, projectCostDetailOperationalDraftSchema) }
  listOperationalDetailDrafts(context: ProjectCostRequestContext, projectId: string) { return this.detailRead('c1_list_project_cost_detail_drafts_operational', context, { target_project_id: projectId }, z.array(projectCostDetailOperationalDraftSchema)) }
