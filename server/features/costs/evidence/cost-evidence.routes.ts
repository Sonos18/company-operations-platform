...
export interface CostEvidenceRouteDependencies { resolveContext(event: H3Event, companyId: string): ReturnType<typeof c1RequestContext>; service?: CostEvidenceService; finalizer?: SupabaseEvidenceFinalizer }
...
export function createCostEvidenceRoutes(dependencies: CostEvidenceRouteDependencies) {
  async function resolved(event: H3Event) { const context = await dependencies.resolveContext(event, param(event, 'companyId')); return { context, service: dependencies.service ?? new CostEvidenceService(new CostEvidenceRepository(context.db, dependencies.finalizer)) } }
  return {
