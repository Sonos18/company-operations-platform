...
export async function run(argv = process.argv.slice(2), dependencies: { env: Record<string, string | undefined>; fetch: typeof fetch } = { env: process.env, fetch: globalThis.fetch }) {
  ...
  if (mode === 'execute' || mode === 'get-result') {
    ...
    let result
    try {
      result = mode === 'execute' ? await command.execute({ execute: argv.includes('--execute'), packet, preparation: json(required(argv, '--preparation')) }) : await command.getResult(packet)
    } catch (error) {
